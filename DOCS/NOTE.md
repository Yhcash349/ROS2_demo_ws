...
# 2026-06-24 Day 8：Nav2 架构、Lifecycle 与参数调试

## 今日学习位置

今天对应 `DOCS/PLAN.md` 的 Stage 2：仿真与导航基础，以及 Day 8：Nav2 架构与低风险参数调试。Day 7 已经从 RViz 跑通手动目标点导航，今天进一步拆解 planner、controller、costmap、goal checker、BT navigator 和 lifecycle manager，并复制 Nav2 官方参数文件进行独立实验。

今天最重要的认识是：Nav2不是一个单独节点，而是一组具有明确职责和启动状态的模块。地图、定位、规划、控制和行为树必须按顺序完成配置并进入 `active`；其中一个关键节点配置失败，RViz 可能仍能显示部分内容，但完整导航 action 不会工作。

## Lifecycle 节点

`ros2 lifecycle nodes` 用来列出实现了 ROS2 Lifecycle 接口的节点，而不是列出全部节点。Lifecycle 节点通常经历 `unconfigured -> inactive -> active -> finalized`：`unconfigured` 表示进程存在但尚未完成参数和插件配置，`inactive` 表示已经配置但未开始工作，`active` 才表示节点正在提供正式功能。

Nav2 使用 lifecycle manager 按顺序配置和激活 `map_server`、AMCL、planner server、controller server、BT navigator 等模块。可以用以下命令检查具体节点：
...
正常运行时，这些关键节点通常应显示 `active [3]`。`ros2 node list` 负责查看全部节点，`ros2 lifecycle nodes` 只关注支持生命周期状态机的节点，两者用途不同。

## 今日理解的低风险参数

当前 ROS2 Jazzy 示例使用 MPPI Controller，因此旧教程里常见的 `max_vel_x`、`max_vel_theta` 在当前文件中对应 `FollowPath.vx_max` 和 `FollowPath.wz_max`。

`vx_max` 是控制器允许生成的最大 X 方向线速度，单位是 `m/s`。调小后机器人前进更慢、更适合稳定观测，但任务时间增加；调大后必须同时考虑底盘能力、地图空间、控制稳定性和制动距离。

`wz_max` 是绕 Z 轴的最大角速度，单位是 `rad/s`。它控制差速机器人转向的激进程度。值过小会导致弯道跟踪能力不足，值过大可能造成旋转过快、路径跟踪不稳定。

`velocity_smoother.max_velocity` 是速度平滑器输出端的上限，三个值依次表示 `[x 方向线速度, y 方向线速度, Z 轴角速度]`。TurtleBot 不能横向平移，所以中间值是 `0.0`。控制器参数决定“控制器想输出多快”，速度平滑器决定“最终最多放行多快”，实际速度受更严格的一层限制。

`inflation_radius` 是 costmap 在障碍物周围建立的高代价安全区域半径，单位是米。增大它会让路径更远离障碍物，但可能使狭窄通道难以通过。local costmap 和 global costmap 中各有一处，分别影响局部避障和全局规划。

`xy_goal_tolerance` 是判断到达目标时允许的平面位置误差，单位是米。例如 `0.25` 表示距离目标点约 25 厘米以内即可判定位置到达。值越小，到点越精确，但机器人可能在终点附近调整更久；朝向误差由 `yaw_goal_tolerance` 单独控制。

## Nav2 框架、Server 与算法插件

Nav2 不是某一个导航算法，而是一套可组合的导航框架。它提供节点通信、统一接口、任务组织、生命周期管理以及一批默认算法实现。框架中的 Server 负责对外提供稳定接口，具体算法通常作为插件加载，因此可以在不重写整套导航系统的前提下更换算法。
...
Nav2 Goal
  -> BT Navigator 组织导航流程
  -> Planner Server 调用规划插件生成全局路径
  -> Controller Server 调用控制插件跟踪路径
  -> Velocity Smoother 限制和平滑速度
  -> /cmd_vel
  -> Gazebo 或真实底盘
```

`planner_server` 不是某个具体规划算法，而是规划插件的运行平台。当前配置加载 `nav2_navfn_planner::NavfnPlanner`；未来可以根据机器人类型和场景更换为其他符合 Nav2 Planner 接口的插件。

`controller_server` 同样不是某个具体控制算法，而是控制插件的运行平台。当前 `FollowPath` 加载的是 `nav2_mppi_controller::MPPIController`。MPPI 只是多种路径跟踪控制算法之一，也可以更换为 DWB 等符合 Nav2 Controller 接口的插件。

MPPI 会在每个控制周期采样并评估多条短期运动轨迹，根据路径偏差、障碍物风险、目标方向等代价选择速度指令。因此 `vx_max`、`wz_max` 等参数是在限制 MPPI 可以选择的速度范围，而不是在改变 Planner 生成的全局路线。

costmap 通过插件层构建导航使用的代价地图。global costmap 主要服务 Planner，local costmap 主要服务 Controller；常见层包括 static layer、obstacle layer 和 inflation layer。Goal Checker 和 Progress Checker 也采用插件机制，分别判断是否到达目标、机器人是否仍在取得有效进展。

BT Navigator 负责组织“规划路径、跟踪路径、重新规划、清理 costmap、执行恢复行为、返回结果”等任务流程。它通常通过行为树 XML 调整流程，不负责直接计算全局路径或底盘速度。

Lifecycle Manager 负责按顺序配置、激活、关闭 Nav2 节点，不参与路径和速度计算。今天 `controller_server` 因参数类型错误无法 configure，正是 Lifecycle Manager 中止导航 bringup、最终导致 `/navigate_to_pose` 不可用的原因。

所谓“插件可以替换”不是随便改一个名字就能运行。更换插件时还要确认插件已安装、实现了对应 Nav2 接口、支持当前差速或阿克曼底盘、参数名称与类型正确，并重新验证 costmap、TF、传感器、速度和行为树配置。

实际机器人导航工作中，这些模块会经常遇到。常见工作不是从零实现整套导航，而是判断问题属于定位、规划、控制、costmap、任务流程还是 lifecycle，再通过调参、更换现有插件、修改行为树或开发自定义插件解决。

## `2D Pose Estimate` 的作用

RViz 的 `2D Pose Estimate` 是给 AMCL 提供机器人在地图中的初始位姿猜测。鼠标按下的位置表示 `map` 坐标系中的初始 X、Y，拖出的箭头表示初始朝向 yaw。它不会移动 Gazebo 中的机器人，也不是导航目标。

AMCL 收到初始猜测后，会结合 `/scan`、`/odom` 和 `/map` 修正定位并建立 `map -> odom`。刚设置时出现绿色粒子云是正常现象；粒子表示 AMCL 认为机器人可能处于的多个位姿，雷达与地图匹配后粒子应逐渐集中。

可以用一句话区分两个 RViz 工具：`2D Pose Estimate` 回答“我现在在哪里”，`Nav2 Goal` 回答“我要去哪里”。

## 踩坑记录

<span style="color:red">用户遇到：加载自定义 `nav2_params.yaml` 后，`controller_server` 在 lifecycle 配置阶段崩溃，RViz 点击 Nav2 Goal 没有反应。</span>

关键日志包括：

```text
controller_server does not have error state implemented
...
根因是将浮点参数 `wz_max: 1.9` 改成了 `wz_max: 1`。YAML 会把 `1` 解析为整数，而 MPPI Controller 对 ROS2 参数类型有严格要求，期望该参数为浮点数。改成 `wz_max: 1.0` 后才能保持正确类型。这说明修改 ROS2 YAML 参数时，不仅数值要合理，`1` 与 `1.0` 的参数类型差异也可能直接导致节点配置失败。

`controller_server` 崩溃后，lifecycle manager 会中止导航栈 bringup，`/navigate_to_pose` action server 因而不可用。RViz 面板按钮或局部状态不能替代终端和 lifecycle 检查；应优先寻找启动日志中第一次出现的参数异常或插件配置错误。

<span style="color:red">用户遇到：RViz 显示 `Fixed Frame [map] does not exist`，RobotModel 下多个 frame 同时变红，地图不显示。</span>

这些通常是同一条上游链路故障的连锁表现：`map` frame 或 `map -> odom` 尚未建立，RViz 无法把 `base_link`、`base_scan` 和相机 frame 转换到 Fixed Frame。应依次检查 `/map`、`map_server`、AMCL、`map -> odom`，而不是逐个处理每个红色 frame。

## 八股自测

Q：`ros2 lifecycle nodes` 和 `ros2 node list` 有什么区别？
A：`ros2 node list` 查看全部 ROS2 节点；`ros2 lifecycle nodes` 只查看实现了 Lifecycle 接口、可以在 `unconfigured`、`inactive`、`active` 等状态间切换的节点。

Q：为什么一个 controller 参数类型错误会让 Nav2 Goal 没反应？
A：controller server 无法完成 configure，lifecycle manager 会中止导航模块 bringup，BT navigator 和 `/navigate_to_pose` action 无法形成完整可用链路，所以 RViz 发出的目标没有可用 action server 接收和执行。

Q：`vx_max` 和 `velocity_smoother.max_velocity` 为什么要一起考虑？
A：`vx_max` 限制 MPPI 控制器生成的速度，`max_velocity` 限制速度平滑器最终输出的速度。两层中较严格的限制决定实际可输出上限，只改其中一层可能看不到预期变化。

Q：MPPI 和 Controller Server 是什么关系？
A：Controller Server 是提供统一路径跟踪接口的运行平台，MPPI 是它当前加载的一种控制算法插件。更换 MPPI 不等于更换整个 Nav2 框架。

Q：Planner Server 和具体 Planner 算法是同一个东西吗？
A：不是。Planner Server 负责接收规划请求并管理插件，NavFn、Smac 等才是生成路径的具体规划插件。

Q：Nav2 的“即插即用”是什么意思？
A：只要算法实现相应 Nav2 插件接口，并与当前机器人模型、依赖、参数和传感器配置匹配，就可以替换具体算法而保留上层导航流程；它不是无需配置和验证的任意替换。

Q：`2D Pose Estimate` 会让机器人移动吗？
A：不会。它只给 AMCL 一个“机器人当前在地图哪里、朝向哪里”的初始猜测；真正让机器人运动的是发送 Nav2 Goal 后，planner、controller 和底盘执行形成的导航链路。

## 明日衔接

下一步进入 Day 9：SLAM 建图与静态地图导航。开始 Day 9 前，应先把 Day 8 的参数实验收束到稳定、低速配置，确认 `map_server`、AMCL、planner、controller 和 BT navigator 均能自动进入 `active`，并完成至少一次可重复的目标点导航。当前参数文件中的速度上限仍处于实验状态，不应直接视为风机巡检推荐值。

# 2026-06-25 Day 9：SLAM 建图、地图保存与静态地图导航

## 今日学习位置

今天对应 `DOCS/PLAN.md` 的 Stage 2：仿真与导航基础，以及 Day 9：SLAM 建图与静态地图导航。Day 7 使用 Nav2 示例自带地图验证“已知地图上的定位和导航”，Day 8 拆解 Nav2 模块和 lifecycle，Day 9 则补齐“地图从哪里来”：机器人使用雷达、里程计和 tf 对环境建图，保存地图后，再切换到 AMCL 和 Nav2 使用这张自建地图。

今天最重要的认识是：SLAM 建图和静态地图导航是两个运行阶段，不能把它们混成同一件事。
...
map.yaml + map.pgm
  -> map_server 发布 /map
  -> AMCL 用 /map、/scan、/odom 完成定位
  -> Planner 和 Controller 执行 Nav2 Goal
```

## 地图文件表示什么

`day9_tb3_map.pgm` 是占据栅格图像，每个像素表示环境中的一小块区域。通常黑色表示占用或障碍物，白色表示已知可通行区域，灰色表示未知区域。

`day9_tb3_map.yaml` 是地图元数据，说明 PGM 文件位置、分辨率、地图原点以及占用和空闲阈值。当前地图的 `resolution: 0.050` 表示一个像素对应现实或仿真环境中的 0.05 米。

地图不是只给 RViz 显示。RViz 只是订阅 `/map` 并把它画出来；AMCL 使用地图定位，global costmap 将静态地图与障碍物层、膨胀层结合，Planner 再在 global costmap 上规划路径。

## SLAM 与 AMCL 的区别

SLAM 回答两个相互关联的问题：“机器人在哪里”和“周围环境长什么样”。在地图未知时，它根据雷达扫描、里程计和机器人运动逐步生成地图。

AMCL 不负责建图。它假设地图已经存在，通过比较当前雷达扫描与静态地图，估计机器人在地图中的位置，并发布 `map -> odom`。

因此，今天保存地图以后重新启动静态导航，不再需要 SLAM Toolbox 持续修改地图，而是使用：

```text
map_server：发布已经保存的地图
AMCL：估计机器人在地图中的位置
Nav2：规划路径并控制机器人到达目标
```
...
```text
启动仿真和 Nav2 节点，但 autostart=False
-> 激活 Localization
-> map_server 发布地图，AMCL 开始工作
-> 设置 2D Pose Estimate
-> 确认 map -> base_footprint
-> 激活 Navigation
-> 发送 Nav2 Goal
```

如果 localization 和 navigation 同时自动激活，AMCL 可能还没有收到初始位姿，`map -> odom` 尚未建立。此时 global costmap 无法把机器人转换到 `map` 坐标系，Planner 会激活失败，后续 BT Navigator 也不会进入 active。

`tf2_echo map base_footprint` 启动瞬间出现一次 `Invalid frame ID "map"` 并不一定代表最终失败。如果随后持续输出 Translation、Rotation 和 Matrix，说明 tf listener 等待之后已经获得完整变换。判断 tf 是否可用要看持续结果，而不是只看第一行。

## Lifecycle 的进一步理解

今天出现了“部分节点 active、部分节点 inactive”的状态。这说明 lifecycle manager 是按列表顺序逐个配置和激活节点的：前面的 Controller 可能已经成功，后面的 Planner 可能因 tf 超时失败，管理器随后停止整个 bringup。

此时不能简单再次发送 STARTUP。STARTUP 会从列表第一个节点重新开始，而 active 节点不能再次执行 configure，所以会出现：
...
更稳妥的方法是停止残缺 launch，重新启动，并在正确的 tf 条件满足后再执行完整 lifecycle 流程。

## ROS2 CLI 与 DDS 状态

今天还遇到 `ros2 service call` 完全没有输出，以及 Fast DDS 报：
...
这不是 lifecycle service 的业务逻辑错误，而是 CLI 在创建 DDS participant 时就卡住了。旧 `ros2-daemon` 和重复启动后残留的 Fast DDS 共享内存锁会导致新 CLI 无法正常加入 ROS 图。

因此应区分两类“命令没反应”：

```text
已经显示 waiting/requester，迟迟无 response
-> 服务存在，但服务端或 DDS 响应异常

连 waiting for service 都没有显示
-> CLI 很可能在 DDS 初始化阶段卡住
```

完全无输出时不要连续重复执行，因为每次都可能增加一个卡住的客户端。应先停止当前命令，再检查 ROS2 daemon、重复进程和 Fast DDS 共享内存占用。

## 今日形成的完整理解

Day 7 证明了“Nav2 能在别人准备好的地图上工作”；Day 9 证明的是“机器人能够对环境建立自己的地图，并将这张地图重新作为定位和导航依据”。
...
对风机巡检项目而言，这意味着未来可以先为巡检场景建立地图，再让机器人围绕塔筒或观测目标执行可重复的导航与采集任务。SLAM 解决“建立环境基准”，AMCL 和 Nav2 解决“后续重复进入环境并执行任务”。

## 八股自测

Q：SLAM 和 AMCL 的核心区别是什么？
A：SLAM 用于未知环境，同时估计机器人位置并建立地图；AMCL 用于已有地图，根据雷达、里程计和静态地图估计机器人位置，不负责生成地图。

Q：PGM 和 YAML 地图文件分别保存什么？
A：PGM 保存占据栅格像素，YAML 保存图像路径、分辨率、原点和占用阈值等元数据。

Q：地图是不是只传给 RViz？
A：不是。RViz 只负责显示；AMCL 用地图定位，global costmap 用地图构建导航代价，Planner 在代价地图上规划路径。

Q：为什么 `map_server` active 但 Planner 仍可能无法 active？
A：地图发布只说明 `/map` 存在。Planner 的 global costmap 还需要 `map -> robot_base_frame` tf；如果 AMCL 尚未建立 `map -> odom`，Planner 仍可能因 tf 超时而激活失败。

Q：为什么不能在部分 Nav2 节点 active 时反复执行 STARTUP？
A：STARTUP 会从管理列表开头重新执行 configure/activate，而 active 节点不支持再次 configure，管理器会在第一个 active 节点处失败。

Q：`tf2_echo` 第一行报 frame 不存在，随后持续输出，应该怎么判断？
A：以后续持续输出为准。第一行可能只是 listener 刚启动时尚未收到 tf；只要随后持续获得变换，说明链路已经建立。
